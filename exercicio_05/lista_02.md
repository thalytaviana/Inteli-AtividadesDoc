...
Var
   idade: inteiro
   votofacultativo, votoobrigatorio: logico
...
Inicio
     ...
     Senao entao idade>=16 e idade<18
      votofacultativo <- verdadeiro
     votoobrigatorio <- falso
          Escreval ("O voto facultativo é:", votofacultativo, "O voto obrigatório é:", votoobrigatorio)
     Fimse
...
```
 Classe FormaGeometrica
    ...
    Classe Retangulo herda FormaGeometrica
        base: Real
        altura: Real
        Metodo Construtor(cor, base, altura)
    ...
    // Criação dos objetos
    retangulo <- Novo Retangulo("vermelho", 5, 10)
    circulo <- Novo Circulo("azul", 7)

    // Saída dos resultados
    Escreva("Área do retângulo: ", retangulo.CalcularArea())
    Escreva("Área do círculo: ", circulo.CalcularArea())
FimAlgoritmo
```
...
```
    var
        velocidadeInicial, aceleracao, velocidadeMaxima, distancia, tempoMaximo, tempo, deslocamento: real

    Escreva("Informe a velocidade inicial do carro (em m/s): ")
    Leia(velocidadeInicial)
    Escreva("Informe a aceleração do carro (em m/s²): ")
    Leia(aceleracao)
    Escreva("Informe a velocidade máxima permitida (em m/s): ")
    Leia(velocidadeMaxima)
    Escreva("Informe a distância da corrida (em metros): ")
    Leia(distancia)
    Escreva("Informe o tempo máximo permitido para a corrida (em minutos): ")
    Leia(tempoMaximo)

    tempoMaximo <- tempoMaximo * 60
    tempo <- 0

    Enquanto distancia > 0 E tempo <= tempoMaximo faca
        velocidade <- velocidadeInicial + aceleracao * tempo
        Se velocidade > velocidadeMaxima entao
            velocidade <- velocidadeMaxima
        FimSe
        deslocamento <- velocidade * 1
        distancia <- distancia - deslocamento
        tempo <- tempo + 1
    FimEnquanto

    Se distancia <= 0 entao
        Escreva("O carro completou a corrida em ", tempo, " segundos.")
    Senao
        Escreva("O carro não conseguiu completar a corrida dentro do tempo máximo permitido.")
    FimSe
FimAlgoritmo
```
______
...
Função MultiplicacaoDeMatrizes(matrizA, matrizB)
    Se tamanho(matrizA[1]) ≠ tamanho(matrizB) então
        Retorne "Impossível multiplicar as matrizes. Dimensões incompatíveis."
    ...
    Para i <- 1 até linhasA faça
        Para j <- 1 até colunasB faça
            soma <- 0
            Para k <- 1 até colunasA faça
